...
class traj_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Save original user_id for result analysis (if available)
        if "original_user_id" in selected:
            return_dict["original_user_id"] = torch.tensor(selected["original_user_id"])
...
def collate_fn(batch):
    ...
    for key in dict_batch:
        if key in ["user", "len", "history_count", "original_user_id"]:
            continue
