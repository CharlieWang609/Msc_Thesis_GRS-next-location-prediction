...
def get_dataset(raw_data_dir, bound_file, epsilon=20, num_samples=1):
    ...
    ## select valid user
    quality_dir = os.path.join("data", "preprocessing", "quality")
    ...
    # Save inputs using the layout consumed by configs/simulation.yml.
    output_dir = Path("data/simulation/input")
    output_dir.mkdir(parents=True, exist_ok=True)
    locs.to_csv(output_dir / "locs.csv", index=False)
    sp.to_csv(output_dir / "loc_seq.csv", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("raw_data_dir", type=str, nargs="?", help="dir for data storage", default="./data/raw")
    parser.add_argument(
        "bound_file",
        type=str,
        nargs="?",
        help="filename of swiss boundary",
        default="./data/geospatial/swiss_1903+.shp",
    )
